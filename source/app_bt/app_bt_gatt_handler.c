...
static uint8_t read_resp_data[20];
                            ...
                            wiced_bt_gatt_event_data_t *p_event_data)
{
    ...
    switch (event)
    {
        ...
        case GATT_DISCOVERY_CPLT_EVT:
            ...
            if(hs_discovery_data.service_found)
            {
    ...
    switch ( p_attr_req->opcode )
    {
        ...
        case GATT_CMD_WRITE:
            ...
            switch (p_attr_req->data.write_req.handle)
            {
                ...
                case HDLD_HELLO_SENSOR_2_NOTIFY_CLIENT_CHAR_CONFIG:
                case HDLD_HELLO_SENSOR_3_NOTIFY_CLIENT_CHAR_CONFIG:
                {
                ...
                case HDLC_HELLO_SENSOR_2_BLINK_VALUE:
                case HDLC_HELLO_SENSOR_3_BLINK_VALUE:
                {
...
app_bt_discovery_result_handler(wiced_bt_gatt_discovery_result_t *discovery_result)
{
    ...
    switch (discovery_type)
    {
        ...
        case GATT_DISCOVER_CHARACTERISTICS:
            ...
            if(!memcmp(uuid_char_hello_sensor_blink, discovery_result->discovery_data.characteristic_declaration.char_uuid.uu.uuid128, MAX_UUID_SIZE))
            {
        ...
        case GATT_DISCOVER_CHARACTERISTIC_DESCRIPTORS:
               ...
               discovery_result->discovery_data.char_descr_info.type.uu.uuid16)
            {
                ...
                hs_discovery_data.service_found = true;
...
app_bt_service_discovery_handler(wiced_bt_gatt_discovery_complete_t *discovery_complete)
{
    ...
    switch (discovery_type)
    {
        case GATT_DISCOVER_SERVICES_BY_UUID:
        {
            ...
            memcpy(&char_discovery_setup.uuid.uu.uuid128[0], &uuid_service_hello_sensor[0], MAX_UUID_SIZE);
        ...
        case GATT_DISCOVER_CHARACTERISTICS:
        {
            char_discovery_setup.s_handle = hs_discovery_data.hs_start_handle;
            char_discovery_setup.e_handle = hs_discovery_data.hs_end_handle;
            memcpy(&char_discovery_setup.uuid.uu.uuid128[0], &uuid_char_hello_sensor_notify[0], MAX_UUID_SIZE);
            gatt_status = wiced_bt_gatt_client_send_discover(discovery_complete->conn_id,
...
app_bt_gatt_operation_cplt_handler(wiced_bt_gatt_operation_complete_t *p_operation_cplt)
{
    ...
    switch(p_operation_cplt->op)
    {
        ...
        case GATTC_OPTYPE_READ_HANDLE:
        {
            ...
            if((p_operation_cplt->status == WICED_BT_GATT_SUCCESS) && (p_operation_cplt->response_data.handle == hs_discovery_data.hs_char_val_blink_handle))
            {
            ...
            if((p_operation_cplt->status == WICED_BT_GATT_SUCCESS) && (p_operation_cplt->response_data.handle == hs_discovery_data.hs_cccd_handle))
            {
                printf("Read response received for cccd\n");
                for(uint8_t i = 0; i < app_gatt_db_ext_attr_tbl_size; i++)
                {
...
void app_bt_read_peer_attributes(uint16_t conn_id)
{
    ...
    if(!peer_info[peer_index].is_read_notify_char_cplt)
    {
        ...
        printf("gatt)status notif = %x\n", gatt_status);
        // printf("Address of p_data = %d\n", &app_gatt_db_ext_attr_tbl[i].p_data);
    }
    if(!peer_info[peer_index].is_read_cccd_cplt)
    {
        ...
        printf("gatt)status cccd = %x\n", gatt_status);
    }
    if(!peer_info[peer_index].is_read_blink_char_cplt)
    {
...
void app_bt_local_gatt_db_info_init(void)
{
    ...
    local_gatt_db_info[1].service_handle = HDLS_HELLO_SENSOR_2;
    local_gatt_db_info[1].is_service_free = TRUE;
    local_gatt_db_info[2].service_handle = HDLS_HELLO_SENSOR_3;
    local_gatt_db_info[2].is_service_free = TRUE;
}
